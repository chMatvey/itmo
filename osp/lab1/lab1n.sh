...
while [ $cont -ne 6 ]
	...
	2)
		...
		read directory
		cd "$directory" 2>> $error_path || echo "Directory does not exist" 1>&2
	;;
	3)
		ls
	;;
	4)
		...
		read linkName
		ln "$fileName" -- "$linkName" 2>> $error_path || echo "Error, please see lab1_err" 1>&2
	;;
	5)
		echo "Enter link name"
		read linkName
		rm "$linkName" 2>> $error_path || echo "Link does not exist" 1>&2
	;;
	6)
		echo "Exit"
	;;
